...
removeTerms <- c("motifFindingParameters.txt", "knownResults.txt","gene3d.txt")
print(filenames <- filenames [! filenames %in% removeTerms])
...
require(export)
## for testing
# dfName <- "kegg"; df <- get(dfName); termNum=10
...
FilterAndPlotGOterms <- function(df, dfName, termNum = 10) {
  ...
  df$Term <- factor(df$Term, levels= unique(as.character(df$Term)))
